...
export const WelcomeCard = ({ resource, className }: WelcomeCardProps) => (
  <Card className={cx('flex flex-col justify-center gap-2 px-6 py-8', className)}>
    ...
    {resource.status === 'error' && <BlockError message={resource.message} onRetry={resource.retry} />}

    {resource.status === 'ready' && (
      <>
        <h1 className="text-h1 text-dark">Welcome {resource.data.greetingName},</h1>
