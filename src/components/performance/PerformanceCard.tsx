...
export const PerformanceCard = ({
  ...
  <Card className={className}>
    <div className="flex items-start justify-between px-4 pt-[17px]">
      <h2 className="text-h5 text-dark">
        {periodLabel ? `${periodLabel}’s performance` : 'Performance'}
      </h2>
      <button
        type="button"
    ...
    {resource.status === 'error' ? (
      <BlockError
        ...
        className="mt-[10px] px-4 pb-[14px]"
      />
    ) : (
      <div className="mt-[10px] flex flex-col px-4 pb-[14px]">
        {inRows(
          ...
          <div
            key={ROW_CLASSES[rowIndex]}
            className={`grid grid-cols-2 gap-x-2 ${ROW_CLASSES[rowIndex]}`}
          >
